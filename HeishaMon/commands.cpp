...
byte optionalPCBQuery[] = {0xF1, 0x11, 0x01, 0x50, 0x00, 0x00, 0x40, 0xFF, 0xFF, 0xE5, 0xFF, 0xFF, 0x00, 0xFF, 0x00, 0xFF, 0xFF, 0x00, 0x00};
...
unsigned int set_optional_pcb(char *msg, unsigned char *cmd, char *log_msg) {
  ...
  {
    char tmp[256] = { 0 };
    snprintf_P(tmp, 255, PSTR("set optional pcb %d"), value - 1);
    memcpy(log_msg, tmp, sizeof(tmp));
  }
...
unsigned int set_heating_mode(char *msg, unsigned char *cmd, char *log_msg) {
  ...
  {
    char tmp[256] = { 0 };
    snprintf_P(tmp, 255, PSTR("set heating mode %d"), value - 1);
    memcpy(log_msg, tmp, sizeof(tmp));
  }
...
unsigned int set_demand_control_enabled(char *msg, unsigned char *cmd, char *log_msg) {
  ...
  {
    char tmp[256] = { 0 };
    snprintf_P(tmp, 255, PSTR("set demand control enabled %d"), value - 1);
    memcpy(log_msg, tmp, sizeof(tmp));
  }
